...
*Setup instructions for a Gitian build of Utabit using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Utabit
...
- [Building Utabit](#building-utabit)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-utabit-host/README.md).
...
Clone the git repositories for utabit and Gitian.
...
in the utabit repository under 'Fetch and build inputs' to install sources which require
...
To build Utabit (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the utabit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/utabit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/utabit/utabit
...
For example:
```bash
URL=https://github.com/laanwj/utabit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit utabit=${COMMIT} --url utabit=${URL} ../utabit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit utabit=${COMMIT} --url utabit=${URL} ../utabit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit utabit=${COMMIT} --url utabit=${URL} ../utabit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the utabit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../utabit/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/utabit/utabit-detached-sigs.git

UBITPATH=/some/root/path/utabit.git
SIGPATH=/some/root/path/utabit-detached-sigs.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/utabit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/utabit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/utabit-osx-build.assert
```
...
[utabit/gitian.sigs](https://github.com/utabit/gitian.sigs/) repository, or if that's not possible create a pull
