Utabit Core version 0.11.0 is now available from:

  <https://utabit.org/bin/utabit-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer UBIT/kB fee to be
...
For example, add the following to `utabit.conf`:
...
related to the blockchain in the utabit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Utabit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Utabit Core will 
...
For an example script see [utabit-submittx](https://github.com/laanwj/utabit-submittx).
...
- #5334 `cf87536` libutabitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new utabit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only UtabitAmountField
- #5753 `0f44672` Add utabit logo to about screen
...
- #5809 `46bfbe7` Add utabit-cli man page
...
- #5849 `41259ca` contrib/init/utabitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Utabit-Qt.app to Utabit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for utabit-tx)
