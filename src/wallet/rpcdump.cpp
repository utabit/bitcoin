...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The utabit address for the private key\n"
            "\nResult:\n"
...
UniValue ProcessImport(const UniValue& data, const int64_t timestamp)
{
    try {
        ...
        if (isP2SH) {
            ...
            CUtabitAddress redeemAddress = CUtabitAddress(CScriptID(redeemScript));
            ...
            if (pubKeys.size() && keys.size() == 0) {
                ...
                CUtabitAddress pubKeyAddress = CUtabitAddress(pubKey.GetID());
                ...
                // Consistency check.
                if (isScript) {
                    CUtabitAddress scriptAddress;
                    CTxDestination destination;

                    if (ExtractDestination(script, destination)) {
                        scriptAddress = CUtabitAddress(destination);
