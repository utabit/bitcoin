...
The RPM spec file provided here is for Utabit-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://utabit.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Utabit git repository](https://github.com/utabit/utabit) but are not part of
...
Currently only `Source10` is used, for the example `utabit.conf` file.
...
in the utabit git repository. At present only an SVG version of the Utabit
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the utabit
...
needed when 0.12.1 is released, a proper fix is already in the Utabit git
master branch.
...
idea to build Utabit Core against a static version of that library compiled
...
The `utabit.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `utabit-server` package. At this point, most RPM based
...
The `utabit-server` package should have SELinux support. How to properly do
...
Utabit-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/utabit/). He has been
packaging Utabit for Fedora at least since 2012.
...
4. I renamed the `utabit` package that contains the Qt GUI to `utabit-core` as
that appears to be how the general population refers to it, in contrast to
`utabit-xt` or `utabit-classic`. I wanted to make sure the general population
